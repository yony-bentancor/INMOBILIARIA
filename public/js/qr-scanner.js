(() => {
  ...
  const scanFrame = async () => {
    ...
    try {
      ...
      // El siguiente intento continúa automáticamente.
    }
  };
  ...
  const startCamera = async () => {
    ...
    if (!detector) {
      setMessage(
        'Tu navegador no permite lectura automática de QR. Podés usar “Tomar foto del QR” o ingresar el código.',
        'warning'
      );
      return;
    }
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setMessage(
        'Este navegador no permite acceder a la cámara. Probá desde Chrome/Edge en HTTPS o usá el código manual.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      setMessage(
        'No pudimos abrir la cámara. Revisá el permiso del navegador o usá “Tomar foto del QR”.',
        'error'
      );
    }
  };
  ...
  photoInput.addEventListener('change', async (event) => {
    ...
    if (!detector) {
      setMessage(
        'Tu navegador no puede leer el QR desde una foto. Ingresá el código manualmente.',
        'warning'
      );
      photoInput.value = '';
      return;
    ...
    try {
      ...
      if (!codes || !codes.length) {
        setMessage('No encontramos un QR en la foto. Probá acercándote un poco más.', 'error');
        return;
      }
