...
exports.owners=(req,res)=>{
  ...
  res.render('admin/owners.njk',{title:'Propietarios | QCASA',owners});
};

exports.ownerNewForm=(req,res)=>res.render('admin/owner-form.njk',{
  title:'Nuevo propietario | QCASA',mode:'create',owner:{active:true}
});
...
exports.ownerDetail=(req,res)=>{
  ...
  res.render('admin/owner-detail.njk',{
    title:`${owner.name} | QCASA`,owner,properties,complaints,alerts,alertsPreview:alerts.slice(0,6),
...
exports.ownerEditForm=(req,res)=>{
  const owner=store.findOwner(req.params.id);
  if(!owner)return res.status(404).send('Propietario no encontrado.');
  res.render('admin/owner-form.njk',{title:`Editar ${owner.name} | QCASA`,mode:'edit',owner});
};
...
exports.properties=(req,res)=>res.render('admin/properties.njk',{title:'Propiedades | QCASA',properties:contextProperties(req)});

exports.propertyNewForm=(req,res)=>{
  const selected=req.adminOwnerContext?.selectedOwner||null;
  res.render('admin/property-form.njk',{
    title:'Nueva propiedad | QCASA',mode:'create',
...
exports.propertyDetail=async(req,res)=>{
  ...
  res.render('admin/property-detail.njk',{
    title:`${p.address} | QCASA`,property:p,
...
exports.propertyEditForm=(req,res)=>{
  const p=store.findProperty(req.params.code);
  if(!p||!propertyAllowed(req,p))return res.status(404).send('Propiedad no encontrada.');
  res.render('admin/property-form.njk',{title:`Editar ${p.address} | QCASA`,mode:'edit',property:p,owners:store.owners.filter(o=>o.active!==false),propertyTypes:PROPERTY_TYPES});
};
...
exports.complaints=(req,res)=>{
  ...
  res.render('admin/complaints.njk',{title:'Reclamos | QCASA',complaints:store.complaints.filter(c=>codes.has(c.propertyCode)).map(c=>({...c,property:store.findProperty(c.propertyCode)}))});
};
...
exports.complaintDetail=(req,res)=>{
  ...
  res.render('admin/complaint-detail.njk',{title:`Reclamo #${c.number} | QCASA`,complaint:c,property:p,technician:t,technicians:store.technicians.filter(x=>x.active),statuses:CLAIM_STATUSES,whatsappUrl});
};
...
exports.alerts=(req,res)=>{
  const codes=contextCodes(req);
  res.render('admin/alerts.njk',{title:'Vencimientos y alertas | QCASA',alerts:alertService.enrichedAlerts().filter(a=>codes.has(a.propertyCode)),alertTypes:ALERT_TYPES,properties:contextProperties(req)});
};
...
exports.documents=(req,res)=>{
  const codes=contextCodes(req);
  res.render('admin/documents.njk',{title:'Documentos | QCASA',documents:store.documents.filter(d=>codes.has(d.propertyCode)),properties:contextProperties(req),documentTypes:DOCUMENT_TYPES});
};
...
exports.settings=(req,res)=>{
  ...
  res.render('admin/settings.njk',{title:'Configuración | QCASA',salaryCalculator:{rentedProperties:rentedProperties.length,totalMonthlyRent,monthlyComplaints:monthlyComplaints.length,openMonthlyComplaints:openMonthlyComplaints.length,monthLabel}});
};
